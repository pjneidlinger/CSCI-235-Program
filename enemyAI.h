...
namespace EnemyAI
{
	 ...
	 */
	string enemyTurn(Tile playersBoard[][BOARD_LENGTH], int hitsRemainingPlayer[]);
...
} // end of EnemyAI namespace

string EnemyAI::enemyTurn (Tile playersBoard [][BOARD_LENGTH],
	int hitsRemainingPlayer[])
{
	//static stack<EnemyAI::FutureShot> shotStack; // hold shots to make near previous hits
	...
	outputMessage = static_cast<char>(shotRow + 'A') + std::to_string(shotCol + 1);
...
void EnemyAI::calculateProbabilityMatrix (const Tile playersBoard [][BOARD_LENGTH],
	int shipLocProbability[][BOARD_LENGTH], int hitsRemaining[])
{
	...
	int bonusLikelyhood; // set higher if we have already hit something
					// in this potential location
	int shipLength; // used to track the ship length of the current ship.

	// Consider every ship that has not been sunk
	for (Tile ship = static_cast<Tile>(1); ship < MISS; ship = static_cast<Tile>(ship + 1))
	{
		...
		{
			for (colIndex=0; colIndex <= BOARD_LENGTH-shipLength; ++colIndex)
			{
				...
				for (posShipLoc = 0; posShipLoc < shipLength; ++posShipLoc)
				{
					...
					// If we have fired a shot here and missed and shank the ship,
					// then the ship cannot fit.
					// There is a little cheat here. A human may not know exactly,
					// which ship a hit of a sunk ship is. However, usually it is obvious.
					if (currentTile == MISS ||
						(currentTile > MISS && hitsRemaining[currentTile-MISS] == 0))
					{
					...
					{
						bonusLikelyhood += 6 - SHIP_SIZE[ship]; // bonus for overlap with hit
					}
				}
				...
				if (isPossible)
				{
					...
					// ship position
					for (posShipLoc = colIndex; posShipLoc < colIndex + shipLength;
						++posShipLoc)
					{
						currentTile = playersBoard[rowIndex][posShipLoc];
						if(currentTile < MISS)
						{
							shipLocProbability[rowIndex][posShipLoc] += bonusLikelyhood;
						}
					}
				}
			}
		...
		{
			for (rowIndex=0; rowIndex <= BOARD_LENGTH-shipLength; ++rowIndex)
			{
				isPossible = true; // Assume possible until we find otherwise
				bonusLikelyhood = 1;
				...
				for (posShipLoc = 0; posShipLoc < shipLength; ++posShipLoc)
				{
					...
					{
						bonusLikelyhood += 6 - SHIP_SIZE[ship]; // bonus for hit on ship
					}
				}
				...
				if (isPossible)
				{
					// Add 1 to the probability of all the locations for this
					// ship position
					for (posShipLoc = rowIndex; posShipLoc < rowIndex + shipLength;
						++posShipLoc)
					{
						currentTile = playersBoard[posShipLoc][colIndex];
						if(currentTile < MISS)
						{
							shipLocProbability[posShipLoc][colIndex] += bonusLikelyhood;
						}
					}
				}
			}
	...
	for (rowIndex = BOARD_LENGTH-1; rowIndex >= 0; --rowIndex)
	{
		for (colIndex = BOARD_LENGTH-1; colIndex >= 0; --colIndex)
		{
			if (matrix[rowIndex][colIndex] >= max)
			{
				...
				highestProbLocations.push_back(rowIndex * BOARD_LENGTH + colIndex);
			}
		}
	}
